...
train_npr.py
============
NPR (Nearest-Neighbor Pixel Residual) baseline — Wang et al., CVPR 2023.

Imports ResNet50 with NPR preprocessing from the official repo:
  baselines/git_repo/NPR-DeepfakeDetection/networks/resnet.py
NPR preprocessing computes x - bilinear_downsample_then_upsample(x) before
feeding to the ResNet backbone.
...
NPR_DIR  = WORK_DIR / 'baselines' / 'git_repo' / 'NPR-DeepfakeDetection'
sys.path.insert(0, str(WORK_DIR))
sys.path.insert(0, str(WORK_DIR / 'src'))
sys.path.insert(0, str(NPR_DIR))
from config import DATA_DIR

from networks.resnet import resnet50 as npr_resnet50
...
def get_args():
    p = argparse.ArgumentParser(description='NPR baseline — 1-ch CT adaptation')
...
def main():
    ...
    if args.out_dir is None:
        args.out_dir = str(WORK_DIR / 'baselines' / 'runs' / f'npr_K{args.K}' / f'trained_on_{mods_tag}')
    ...
    # ── Model ─────────────────────────────────────────────────────────────
    model = npr_resnet50(pretrained=False, num_classes=1).to(device)
    print(f"NPR-ResNet50  params={sum(p.numel() for p in model.parameters()):,}")
    ...
    if args.use_wandb:
        ...
        wandb.init(project='MedForensics-baselines',
                   name=args.run_name or f"npr_K{args.K}_{mods_tag}", config=args_dict)
