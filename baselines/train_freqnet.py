...
train_freqnet.py
================
FreqNet baseline — Tan et al., AAAI 2024.

Imports FreqNet from the official repo:
  baselines/git_repo/FreqNet-DeepfakeDetection/networks/freqnet.py
FreqNet uses frequency-domain analysis layers (DCT decomposition) on top of
a ResNet-style backbone for deepfake detection.
...
FREQNET_DIR  = WORK_DIR / 'baselines' / 'git_repo' / 'FreqNet-DeepfakeDetection'
sys.path.insert(0, str(WORK_DIR))
sys.path.insert(0, str(WORK_DIR / 'src'))
sys.path.insert(0, str(FREQNET_DIR))
from config import DATA_DIR

from networks.freqnet import freqnet as build_freqnet
...
def evaluate_slices(model, loader, device) -> tuple:
    ...
    with torch.no_grad():
        for b in loader:
            imgs   = b['image'].to(device).repeat(1, 3, 1, 1)
...
def evaluate_volumes(model, loader, device) -> dict:
    ...
    labels, scores, mods = [], [], []
    for b in loader:
        imgs = b['images'].squeeze(0).to(device).repeat(1, 3, 1, 1)
        logits = model(imgs).squeeze(1)
...
def get_args():
    p = argparse.ArgumentParser(description='FreqNet baseline — 1-ch CT adaptation')
...
def main():
    ...
    if args.out_dir is None:
        args.out_dir = str(WORK_DIR / 'baselines' / 'runs' / f'freqnet_K{args.K}' / f'trained_on_{mods_tag}')
    ...
    # ── Model ─────────────────────────────────────────────────────────────
    model = build_freqnet().to(device)
    print(f"FreqNet  params={sum(p.numel() for p in model.parameters()):,}")
    ...
    if args.use_wandb:
        ...
        wandb.init(project='MedForensics-baselines',
                   name=args.run_name or f"freqnet_K{args.K}_{mods_tag}", config=args_dict)
