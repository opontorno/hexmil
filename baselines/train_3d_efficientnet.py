#!/usr/bin/env python3
"""
train_3d_efficientnet.py
========================
Training script for the EfficientNet 3D CNN volume baseline.
...
Usage
-----
    python baselines/train_3d_efficientnet.py \
        --data_dir /mnt/.../M3DSynth \
        --out_dir  baselines/runs/efficientnet3d_b0_K16 \
...
from models.efficientnet3d_classifier import build_efficientnet3d_classifier
...
class Volume3DDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        # Convert to tensor (1, K, H, W) and resize to 224
...
def get_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Train EfficientNet 3D baseline')
    p.add_argument('--data_dir', type=str, required=True)
    p.add_argument('--out_dir', type=str, required=True)
    p.add_argument('--model_name', type=str, default='efficientnet-b0',
                   choices=VALID_EFFICIENTNETS)
...
def main():
    ...
    # Model
    model = build_efficientnet3d_classifier(
        model_name=args.model_name,
        pretrained=args.pretrained,
        dropout=args.dropout,
    ).to(device)
    print(
        f'Model: {args.model_name}_3d  '
    ...
    if args.use_wandb:
        ...
        run_name = args.run_name or f"{args.model_name.replace('-', '_')}_3d_K{args.K}"
