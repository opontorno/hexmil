...
from models.mvit_classifier import build_mvit_classifier, VALID_ARCHS
...
def get_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Train MViT-V2 baseline')
    p.add_argument('--data_dir',     type=str,   required=True)
    p.add_argument('--out_dir',      type=str,   required=True)
    p.add_argument('--arch',         type=str,   default='mvit_v2_s',
                   choices=list(VALID_ARCHS),
                   help='mvit_v1_b (36.6M), mvit_v2_s (34.5M), mvit_v2_b (51.2M)')
    p.add_argument('--K',            type=int,   default=16,
                   help='Number of slices per volume window (MViT designed for T=16)')
