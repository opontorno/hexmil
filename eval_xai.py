...
from hexmil.utils.xai_utils import (
    _get_ty, _normalize01, _smooth,
    _patch_cams_to_heatmap, _FeatureStore, _register_gradcam_hook,
    _abmil_heatmap as _abmil_heatmap_base, _gradcam_heatmap,
    _bootstrap_ci, _extended_stats, _bh_fdr,
    _pairwise_tests, _summarize, _save_paper_figures,
)
...
_spec = importlib.util.spec_from_file_location('train_slicemil_bench', _TRAIN_PY)
...
def _abmil_heatmap(model, patches, grid_hw, slice_hw, patch_size, stride, device, sigma_frac):
	return _abmil_heatmap_base(
		model, patches, grid_hw, slice_hw, patch_size, stride, device, sigma_frac,
		reconstruct_heatmap_fn=reconstruct_heatmap,
	)
...
def main() -> None:
	...
	patch_size = int(train_args.get('patch_size', 64))
