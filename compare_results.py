...
def _pretty(df: pd.DataFrame) -> str:
    ...
    float_cols = [c for c in disp.columns
                  ...
                  and c not in ("proj_dim", "attn_dim", "vol_attn_dim",
                                "slice_attn_dim", "K", "patch_size", "stride")]
    for c in float_cols:
        disp[c] = disp[c].apply(
            lambda x: f"{x:.4f}" if isinstance(x, float) and not np.isnan(x) else "—"
        )
    for c in ("K", "epoch", "proj_dim", "attn_dim", "vol_attn_dim", "slice_attn_dim"):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Produce slices.csv and volumes.csv from ABMIL runs/"
    )
