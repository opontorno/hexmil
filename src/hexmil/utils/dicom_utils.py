...
dicom_utils.py
--------------
DICOM loading utilities used by scripts/preprocess_ctgan.py.
"""
...
def get_spacing_orientation_dicom_scan(dicom_folder):
    ...
    with pydicom.dcmread(fname) as fid:
        ...
        spacing     = np.array([spacing_z, spacing_xy[1], spacing_xy[0]], dtype=np.float32)
        orientation = np.transpose(fid.ImageOrientationPatient)
...
def load_dicom_scan(dicom_folder):
    ...
    for dcm in os.listdir(dicom_folder):
        if dcm.endswith('.dcm'):
            with pydicom.dcmread(os.path.join(dicom_folder, dcm)) as slice_data:
                slices.append(slice_data.pixel_array)
                indexes.append(-1 * float(slice_data.ImagePositionPatient[2]))

    return np.stack([x for _, x in sorted(zip(indexes, slices))], 0)
