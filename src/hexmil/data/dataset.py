...
import random
import pandas
from hexmil.utils.tiff_utils import load_slice_tiff_scan, get_shape_tiff_scan
from hexmil.utils.tiff_utils import get_percentile_tiff_scan, apply_percentile
from torch.utils.data.dataset import Dataset
from torch import from_numpy as numpy2torch


def get_table_date(data_dir, name_set, mods):
    tab = pandas.read_csv(os.path.join(data_dir, 'data.csv'))
    tab = tab.merge(pandas.read_csv(os.path.join(data_dir, 'sets.csv')))
    tab = tab.merge(pandas.read_csv(os.path.join(data_dir, 'centers.csv')))
...
class M3Dsynth(Dataset):
    def __init__(self, M3Dsynth_dir, tab, output_shape, random_crop=True, aug_fun=None):
        ...
        if random_crop:
            ...
            if 2*np.max(absdiff(self.tab['coord_z'], self.tab['center_test_z'])) > self.output_shape[0]:
                self.tab['center_test_z'] = self.tab['coord_z']
            
            if 2*np.max(absdiff(self.tab['coord_y'], self.tab['center_test_y'])) > self.output_shape[1]:
                self.tab['center_test_y'] = self.tab['coord_y']
            
            if 2*np.max(absdiff(self.tab['coord_x'], self.tab['center_test_x'])) > self.output_shape[2]:
