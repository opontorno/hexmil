#!/usr/bin/env python3
"""
patch_feature_analysis.py
=========================
Analisi delle feature estratte dalle patch di nodulo (Phase A).
...
Il modello viene selezionato sul validation set e valutato sul test set.

Output:
  scripts/results/patch_feature_analysis.json   – metriche complete
  scripts/results/patch_feature_analysis.csv    – tabella riassuntiva

Usage:
    python scripts/patch_feature_analysis.py [--patch_size 128] [--pca_dim 128]
"""
...
def get_args():
    p = argparse.ArgumentParser(description='Patch feature analysis — ML classifiers')
    p.add_argument('--patch_size', type=int, default=128,
                   help='Patch size (default 128, use 64 for faster run)')
...
def extract_raw(patch: np.ndarray) -> np.ndarray:
    """Flatten pixel values. patch: (H, W) float32 in [0,1]."""
    return patch.ravel()

def extract_fourier(patch: np.ndarray) -> np.ndarray:
    """
    2D FFT magnitude spectrum, log-scaled and flattened.
    Captures texture/periodicity differences between real and fake.
    """
    spec = np.abs(fftshift(fft2(patch)))
    spec = np.log1p(spec)
    return spec.ravel()

def extract_dct(patch: np.ndarray) -> np.ndarray:
    """
    2D DCT-II coefficients (all channels), flattened in zig-zag order.
    DCT concentrates energy in low frequencies; GAN artifacts often show
    as anomalous high-frequency residuals.
    """
    coeff = dctn(patch, norm='ortho')
    # zig-zag traversal to keep low-freq components first
    H, W = coeff.shape
    zigzag = []
...
def extract_wavelet(patch: np.ndarray, wavelet: str = 'haar', level: int = 2) -> np.ndarray:
    """
    2D Discrete Wavelet Transform (DWT), concatenate all sub-band coefficients.
    Captures multi-scale spatial frequency information.
    """
...
def load_split(
    ...
    """
    Load all patches for a split. Returns:
        images:  (N, H, W) float32 numpy array
        labels:  (N,) int array  0=real  1=fake (binary)
    ...
    print(f"  [{split:>5}] Loading {len(ds)} patches{desc}...", end='', flush=True)
    t0 = time()

    images  = np.empty((len(ds), patch_size, patch_size), dtype=np.float32)
    labels  = np.empty(len(ds), dtype=np.int32)
    mod_list = []
    id_list  = []

    for i, sample in enumerate(ds):
        images[i]  = sample['image'].squeeze(0).numpy()
        labels[i]  = 1 if sample['label'] > 0 else 0
        mod_list.append(sample['mod'])
        id_list.append(sample['img_id'])

        if (i + 1) % 1000 == 0:
            print(f"\r  [{split:>5}] {i+1}/{len(ds)} patches loaded...", end='', flush=True)

    print(f"\r  [{split:>5}] {len(ds)} patches loaded in {time()-t0:.1f}s  "
...
def main():
    ...
    print(f"\n{'='*65}")
    print(f"  Patch Feature Analysis  (patch_size={args.patch_size}  pca_dim={args.pca_dim})")
    print(f"{'='*65}\n")

    # ── Load raw images ────────────────────────────────────────────────────
    print("Loading data splits...")
    imgs_tr,  labs_tr,  mods_tr,  ids_tr  = load_split('train', args.patch_size, args.max_train)
    imgs_val, labs_val, mods_val, ids_val  = load_split('valid', args.patch_size)
    imgs_te,  labs_te,  mods_te,  ids_te   = load_split('test',  args.patch_size)
    print()
    ...
    for feat_name, extractor in FEATURE_EXTRACTORS.items():
        ...
        for clf_name, pipeline in clfs.items():
            ...
            # Validate (used for model selection / debugging)
    ...
    # ── Save results ───────────────────────────────────────────────────────
    json_path = RESULTS_DIR / f'patch_feature_analysis_p{args.patch_size}.json'
    csv_path  = RESULTS_DIR / f'patch_feature_analysis_p{args.patch_size}.csv'
    ...
    # ── Print summary table ────────────────────────────────────────────────
    print(f"\n{'='*65}")
    print(f"  SUMMARY TABLE  (test set, patch_size={args.patch_size})")
    print(f"{'='*65}")
