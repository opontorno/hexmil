...
def get_args() -> argparse.Namespace:
    ...
    p.add_argument('--sa_n_heads',  type=int, default=8,
                   help='Number of self-attention heads in SliceTransformer '
                        '(slice embedding dim must be divisible by sa_n_heads)')
    p.add_argument('--sa_n_layers', type=int, default=2,
                   help='Number of Transformer encoder layers in SliceTransformer')
...
def main(args: argparse.Namespace) -> None:
    ...
    if args.mods is None:
        _slice_fake = [m for m in _slice_args.get('mods', []) if m != 'real']
        if not _slice_fake:
    ...
    base_dir      = Path(WORK_DIR) / 'experiments' / 'SelfAttention' / 'runs' / f"volume-cls_{backbone_tag}_p{patch_size}_s{stride}_K{args.K}{attn_tag}"
